...
TEST(Activation, softmax)
{
	...
	std::vector<float> res = Ag::Activation::softmaxFunction(inputs);
	std::transform(res.begin(), res.end(), res.begin(), [](const float& z)-> auto {return static_cast<float>(std::truncf(z*100))/100; }); // trunc at 10^-2
...
{
	const std::vector<float> inputs = { 1,3,2.5,5,4,2 };
	std::vector<float> res = Ag::Activation::identityFunction(inputs);
